...
async fn list_providers() -> Result<()> {
    ...
    println!("{:<15} {:<25} {:<15} {:<30}", "ID", "NAME", "STATUS", "CATEGORIES");
    println!("{}", "-".repeat(90));
    ...
    for provider in registry.list_providers() {
        ...
        println!("{:<15} {:<25} {:<15} {:<30}", d.id, d.display_name, status, categories.join(", "));
    }
...
async fn connect_provider(provider_id: &str) -> Result<()> {
    ...
    println!("Run `infs app list --provider {}` to see available apps.", provider_id);
...
async fn show_provider(provider_id: &str) -> Result<()> {
    ...
    let auth_methods: Vec<String> = provider.supported_auth_methods().iter().map(|m| m.to_string()).collect();
